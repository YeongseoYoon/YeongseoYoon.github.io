...
describe('방류', () => {
  ...
  it('TC-4-6 같은 날 여러 생물을 제한 없이 방류할 수 있다', async () => {
    const created = await Promise.all(
      Array.from({ length: 6 }, (_, i) =>
        releaseCreature({ ...base, name: `물고기${i}`, sprite: drawing() }),
      ),
    );
    expect(created).toHaveLength(6);
    expect(created.every((creature) => creature.status === 'published')).toBe(true);
  });

  it('TC-4-8 임시저장은 draft로 남는다', async () => {
