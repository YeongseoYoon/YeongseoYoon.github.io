...
- `Screen` 셸 세 종류: `full`(탐험 지도), `content`(읽기 중심 화면), `wide`(그리기·수조처럼 데스크톱에서 2열로 확장되는 화면)
...
- **공간 범위 조회**: 서버에서는 전체 생물 수와 가장 먼 좌표만 경량 조회하고, 작품 본문은 현재 카메라 주변만 최대 300마리씩 내려받는다.
...
- 오래 탐험한 헤엄 상태와 래스터 캐시는 각각 상한을 두어 메모리가 무한히 늘지 않는다.

### 저장 (`shared/lib/spriteCodec.ts`, `shared/api/localStore.ts`)
- 스프라이트는 **팔레트 + RLE 문자열**로 직렬화한다. 36×32 그림이 실측 **51자** (JSON 배열 대비 수백 배 절감).
  별도 이미지 파일을 업로드하지 않고 Supabase `creatures.sprite`의 TEXT 컬럼에 이 문자열을 저장한다.
...
| `/tank/:authorId` | 공유 수조 | 창작자의 공개 생물을 모아 보는 읽기 전용 수족관 |
...
- 인스타그램 버튼은 공유 링크를 복사하고 스토리 링크 스티커로 붙이는 방법을 안내합니다. 이미지 파일을 만들거나 다운로드하지 않습니다.
